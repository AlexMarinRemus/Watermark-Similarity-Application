...
const get_image_path = "http://localhost:5000/get_threshold_image";
const submit_image_path = "http://localhost:5000/submit_threshold_image";
const load_image_path = "http://localhost:5000/load_threshold_images";
...
load_images()
    .then(async () => {
        ...
        promises = [];
        for (let i = 0; i < 6; i++) {
            promises.push(get_threshold_image(i));
        }
...
// Asynchronous function that gets a specific index of a thresholded image from the
// backend
async function get_threshold_image(option) {
    console.log("Option ", option);
    // Sends which number of thresholded image is desired to the backend
